...
- **ステータス:** 採択（capture / trim フェーズは [ADR-0004](0004-page-acquisition.md) で置換）
- **日付:** 2026-08-08

> **[ADR-0004](0004-page-acquisition.md) による更新:** スパイク1・2 は実測済み。
> ページはサーバ側でレンダリングされた画像として配信されており、DOM に本文は存在しない。
> capture は「スクリーンショット」ではなく「ページ画像の原寸取得」になり、
> **`trim` フェーズは削除された**（取得画像に UI が写らないため）。
> ページ遷移は DOM のページ番号で確定検出する。
